package LeetCode.knapSack_Variety;
...
public class Profitable_Scheme implements Runnable {
	...
	static final FastReader fr = new FastReader(
			"/home/atish/eclipse-workspace/Dynamic_Programming_DP/src/LeetCode/Knapsack_Variety/knapsack.txt");

	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Profitable_Scheme(), "Main", 1 << 26).start();
	}

	static int dp[][][] = new int[101][101][101];;

	public int f(int a[], int b[], int n, int i, int cp, int mp) {

		if ((cp >= mp && i == a.length) || (cp >= mp && n == 0))
			return 1;
		...
		if (a[i] <= n) {
			...
			return dp[n][i][cp] = (f(a, b, n - a[i], i + 1, Math.min(cp + b[i], mp), mp) + f(a, b, n, i + 1, cp, mp))
					% mod;

		}

		return dp[n][i][cp] = f(a, b, n, i + 1, cp, mp) % mod;
	}

	void solve() {
		int n = fr.nextInt(), l = fr.nextInt(), minProfit = fr.nextInt(), gr[] = fr.nextIntArray(l),
				pr[] = fr.nextIntArray(l);
		for (int x[][] : dp) {
			for (int y[] : x)
				Arrays.fill(y, -1);
		}
		out.print(f(gr, pr, n, 0, 0, minProfit));
