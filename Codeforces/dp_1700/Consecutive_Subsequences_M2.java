...
public class Consecutive_Subsequences_M2 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Consecutive_Subsequences_M2(), "Main", 1 << 26).start();
	}
	...
	void solve() {
		...
		for (int i = 0; i < n; ++i) {
			int x = a[i];
			int val = mp.getOrDefault(x - 1, 0) + 1;
			mp.put(x, val);
			if (max < val) {
				max = val;
				last = x;
