...
public class Password_M2 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Password_M2(), "Main", 1 << 26).start();
	}
	...
	void solve() {
		String s = fr.next();
		int z[] = z_function(s), n = s.length();
		int f[] = new int[n];

		int resultant_length = -1, max = NINF;
		for (int i = 1; i < n; ++i) {
			if (z[i] == n - i && max >= z[i]) {
				resultant_length = z[i];
				break;
			}
			max = Math.max(max, z[i]);
		}

		out.print(resultant_length == -1 ? "Just a legend" : s.substring(0, resultant_length));
