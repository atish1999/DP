...
public class Flowers_D_top_Down implements Runnable {
...
//			"/home/atish/eclipse-workspace/Dynamic_Programming_DP/src/Aditya_Verma/concept/MCM/input.txt");
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Flowers_D_top_Down(), "Main", 1 << 26).start();
	}

	int dp[];
	int sum[];

// state dp[i] means no. of ways we can eat i flowers consisting of
// red and white flowers where we can eat white flowers only in the 
// group of size k
	void solve() {
		int n = fr.nextInt(), k = fr.nextInt();
		dp = new int[N + 1];
		sum = new int[N + 1];
//i.e. for each length upto k-1 ans is always 1 because in such case
//she can eat only red flower
		Arrays.fill(dp, 0, k, 1);
// if i>=k means for ith flower if she ate red flower after that 
// she can eat any flower but if she ate white flower then she has
//	to eat another (k-1) white flowers in contiguos fashion
		for (int i = k; i <= N; i++)
		...
		while (n-- > 0) {
			...
			out.print((((sum[b] - sum[a - 1]) + mod) % mod) + "\n");
		}
