...
public class Frog_1 implements Runnable {
	...
	static final FastReader fr = new FastReader("D://DP/src/Atcoder/input1.txt");
	static final PrintWriter out = new PrintWriter(new BufferedOutputStream(System.out));

	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Frog_1(), "Main", 1 << 26).start();
	}
...
/*
	...
	int bup(int h[], int n) {
		...
		return dp[n] = Math.min(Math.abs(h[n] - h[n - 1]) + bup(h, n - 1),
				Math.abs(h[n - 2] - h[n]) + bup(h, n - 2));
	}
