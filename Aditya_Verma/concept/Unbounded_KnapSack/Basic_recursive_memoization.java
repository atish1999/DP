...
public class Basic_recursive_memoization implements Runnable {
...
/*
	...
	with certain value vali and weight wt[],
	we need to calculate minimum amount 
	that could make up this quantity exactly. 
	or we need to calculate the maximum profit that
	we are able to collect from this situation
	...
	Input :
	       ...
	       val[]  = {1, 30}
	       wt[] = {1, 50}
	Output : 100 (maximum profit we can earn)
 ...
 */
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Basic_recursive_memoization(), "Main", 1 << 26).start();
	}
	int unbounded_knapsack(int wt[], int val[], int n, int W) {
		if(n==0 || W==0) return dp[n][W]=0;
		if(dp[n][W]!=-1) return dp[n][W];
		if(val[n-1]<=W) {
			return dp[n][W]=Math.max((val[n-1]+unbounded_knapsack(wt,val,n,W-val[n-1]))
			,unbounded_knapsack(wt,val,n-1,W));
		}else {
