...
public class Count_All_Common_Subsequences implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Count_All_Common_Subsequences(), "Main", 1 << 26).start();
	}
	...
	int f(String s1, String s2, int n, int m) {
		...
		if (s1.charAt(n - 1) == s2.charAt(m - 1))
			return dp[n][m] = 2 * f(s1, s2, n - 1, m - 1);

		return dp[n][m] = f(s1, s2, n - 1, m) + f(s1, s2, n, m - 1) - f(s1, s2, n - 1, m - 1);
	}
