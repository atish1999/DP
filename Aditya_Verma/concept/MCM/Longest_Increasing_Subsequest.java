...
public class Longest_Increasing_Subsequest implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Longest_Increasing_Subsequest(), "Main", 1 << 26).start();
	}
	...
	void solve() {
		...
		if (n == 1) { // if we have one element then longest Increasing Subsequence would be 1
			out.print(1);
			return;
		}
		dp = new int[n];
//		because in worst case answer would be 1 i.e. when all the elements will be in decreasing
//		order i.e. 7 6 5 4 3 2 1 in this case answer would contain only 1 number i.e. 7
		Arrays.fill(dp, 1);
		...
		for (int i = 1; i < n; i++) {
			for (int j = 0; j < i; j++) {
				if (a[j] <= a[i]) { // longest non decreasing subsequence
//		If we are able to add a[j] in this longest Increasing subsequence then we will be adding
//		1 into dp[j].after that we are checking whether we can improve this temporary answer or not

//		dp[i] denotes the length of the LIS up to that a[i]-th element
