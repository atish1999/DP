...
public class Basic_Recursive_and_bottom_up implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Basic_Recursive_and_bottom_up(), "Main", 1 << 26).start();
	}
/*
 ...
 */
	...
	int fun(int a[], int i, int j) { // bottom up approach
/*
 * i==j means there is 1 element in the array and i>j means there is no
 * element in the array
 * in such cases answer should be 0 beacuase in i>j as there is no element
 * in the array we will not be able to construct matrix and also for
 * i==j beacuse we are forming matrix with order (a[i-1] * a[i])
 */
		if (i >= j) 
			return 0;
	...
	void solve() {
		...
		for (int x[] : dp)
			Arrays.fill(x, -1);
/*i will be starting from 1 because in that case matrix will
be a[i-1]*a[i] i.e. a[0]*a[1] and 
j would be n-1 and mmatrix would be  a[n-2]*a[n-1]
*/
		out.print(fun(a, 1, n - 1));
	}
