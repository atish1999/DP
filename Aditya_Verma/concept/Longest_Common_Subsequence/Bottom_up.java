...
public class Bottom_up implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Bottom_up(), "Main", 1 << 26).start();
	}

	static int dp[][];

	int bottom_up(char[] a, char[] b, int n, int m) {
		if (n == 0 || m == 0)
			return 0;
		if (dp[n][m] != -1)
			return dp[n][m];
		if (a[n - 1] == b[m - 1]) {
			return dp[n][m] = 1 + bottom_up(a, b, n - 1, m - 1);
		}
		return dp[n][m] = Math.max(bottom_up(a, b, n - 1, m), bottom_up(a, b, n, m - 1));
	}
	...
	void solve() {
		...
		int max_length=bottom_up(a, b, n, m);
