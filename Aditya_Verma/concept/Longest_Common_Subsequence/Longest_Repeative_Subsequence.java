...
public class Longest_Repeative_Subsequence implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Longest_Repeative_Subsequence(), "Main", 1 << 26).start();
	}
	/*
	 ...
	 * have the same index in the original string. Example: Input: str = "aab"
	 * Output: "a" The two subsequence are 'a'(first) and 'a' (second). Note that
	 * 'b' cannot be considered as part of subsequence as it would be at same index
	 * in both.
	 * 
	 * or a better understanding consider this case ABCABCA here the ans is 4
	 * because of ABCA, here A at the 4 th position is shared, key point is position
	 * of the element in the next repeating subsequence can't be the same. for 1st
	 * seq 1st A of string is at 1st place of subseq, and 4 A of string is at 1st
	 * position of 2nd subseq.
	 */
	...
	void solve() {
		...
		int m = dummy.length();
		// dummy is nothing but same as a
		dp = new int[n + 1][m + 1];
		Arrays.fill(dp[0], 0);
		for (int i = 1; i < m + 1; i++)
			dp[i][0] = 0;
		...
		out.print(dp[n][m] + "\n");
		StringBuilder sb = new StringBuilder();// po=new StringBuilder();
		int i = n, j = m;
//		out.print(i+" "+j+"\n");
		while (i > 0 && j > 0) {
			if (a[i - 1] == dummy.charAt(j - 1) && i != j) {
