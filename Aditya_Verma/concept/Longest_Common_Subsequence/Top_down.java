...
public class Top_down implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Top_down(), "Main", 1 << 26).start();
	}
	...
	void solve() {
		...
		dp = new int[n + 1][m + 1];
		/*
		 * if there is any one or both the strings are empty then longest common
		 * subsequence length would be 0
		 */
		for (int i = 1; i <= n; i++)
			dp[i][0] = 0;
		Arrays.fill(dp[0], 0);
		...
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= m; j++) {
				...
				/*
				 * dp[i][j] = (a[i - 1] == b[j - 1]) ? 1 + dp[i - 1][j - 1] : Math.max(dp[i -
				 * 1][j], dp[i][j - 1]);
				 * 
				 */
			}
		...
		out.print("max length would be " + dp[n][m]);
